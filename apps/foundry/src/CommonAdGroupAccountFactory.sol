...
import "./CommonAdGroupAccount.sol";

/**
 * A sample factory contract for CommonAdGroupAccount
...
contract CommonAdGroupAccountFactory {
    ...
    function createAccount(
        address owner,
        uint256 salt
    ) public returns (CommonAdGroupAccount ret) {
        address addr = getAddress(owner, salt);
        uint256 codeSize = addr.code.length;
        if (codeSize > 0) {
            return CommonAdGroupAccount(payable(addr));
        }
        ret = CommonAdGroupAccount(
            payable(
